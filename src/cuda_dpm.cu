    ...
    for (int gj = 0; gj < d_numVertices; gj++) {
      ...
      if (dx < sij) {
        ...
        if (dy < sij) {
          ...
          if (rij < sij) {
            printf("gi %d interacting with gj %d at positions %f \t %f, %f \t %f with sij %f and force %f %f, rho0 %f\n", vertexID, gj, pos[NDIM * gi], pos[NDIM * gi + 1], pos[NDIM * gj], pos[NDIM * gj + 1], sij, fx, fy, d_rho0);
            //  force scale
